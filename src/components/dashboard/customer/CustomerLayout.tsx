...
const navigation = [
  ...
  { name: "KYC Verification", href: "/customer/kyc", icon: Shield },
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center gap-3 px-6 py-6 border-b border-emerald-800/30">
            ...
            />
            <span className="font-display text-xl font-bold text-white">Veridian</span>
          </div>
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-amber-500 to-yellow-500 flex items-center justify-center">
                <span className="text-emerald-950 font-bold text-sm">
                  {userExtended?.full_name?.charAt(0) || "U"}
                </span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-medium text-white truncate text-sm">
                  {userExtended?.full_name || "Customer"}
                </p>
                <p className="text-emerald-400/70 text-xs">Personal Account</p>
              </div>
            </div>
            {/* KYC Status Badge */}
            <div className="mt-3 inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/30 rounded-full">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
              <span className="text-emerald-400 text-xs font-medium">
                KYC {userExtended?.kyc_status || "pending"}
              </span>
            </div>
          </div>
